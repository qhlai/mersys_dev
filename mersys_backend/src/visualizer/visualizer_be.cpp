...
{
    std::string marker_topic = "mersys_markers";
    std::string cloud_topic = "mersys_cloud";
    std::string odom_topic = "mersys_odom";
...
auto Visualizer::CreatePoint3D(Eigen::Vector3d &p3D, size_t client_id)->pcl::PointXYZRGB {
    pcl::PointXYZRGB p;
    p.x = mersys_params::vis::scalefactor*p3D(0);
    p.y = mersys_params::vis::scalefactor*p3D(1);
    p.z = mersys_params::vis::scalefactor*p3D(2);

    if(client_id < 12) {
        p.r = static_cast<uint8_t>(mersys_params::colors::col_vec[client_id].mu8R);
        p.g = static_cast<uint8_t>(mersys_params::colors::col_vec[client_id].mu8G);
        p.b = static_cast<uint8_t>(mersys_params::colors::col_vec[client_id].mu8B);
    } else {
        p.r = 127;
        p.g = 127;
        p.b = 127;
...
auto Visualizer::PubTrajectories()->void {
    ...
    for(std::set<int>::iterator sit = pccids.begin(); sit != pccids.end(); ++sit){
        ...
        traj.scale.x=mersys_params::vis::trajmarkersize;
        traj.action=visualization_msgs::Marker::ADD;

        // traj.color = MakeColorMsg(0.5,0.5,0.5);
        if(cid < 12){
            mersys_params::VisColorRGB col = mersys_params::colors::col_vec[cid];
    ...
    precision_t scale = mersys_params::vis::scalefactor;
...
auto Visualizer::PubLoopEdges()->void {
    ...
    msg_intra.scale.x = mersys_params::vis::loopmarkersize;
    msg_intra.id = 0;
    ...
    msg_inter.scale.x = mersys_params::vis::loopmarkersize;
    msg_inter.id = 1;
...
auto Visualizer::Run()->void{
    ...
    while(1) {
        ...
        //     if(mersys_params::vis::showkeyframes)
        //         this->PubKeyframesAsFrusta();

        //     if(mersys_params::vis::showtraj)
        //         this->PubTrajectories();

        //     if(mersys_params::vis::showlandmarks)
        //         this->PubLandmarksAsCloud();

        //     if(mersys_params::vis::showcovgraph)
        //         this->PubCovGraph();
