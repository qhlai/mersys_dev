...
public:
  ...
  Calibration(const std::string& CamCfgPaths, const std::string& CalibCfgFile, bool use_ada_voxel) //const PointCloudEXList& lidar_cloud_, const ImageEXList& camera_//
  {
...
// ROS_INFO_STREAM("roughCalib1.2");
    for(size_t i = 0; i < lidar_edge_clouds_3d->size(); i++)
    {
      ...
      #ifdef FISHEYE
        if(cos_angle_1(q_ * pt1 + t_, pt2) > cos(DEG2RAD(cam_fov_/2.0))) // fisheye cam FoV check
          pts_3d.emplace_back(cv::Point3f(pt1(0), pt1(1), pt1(2)));
      #else
        if(cos_angle_1(q_ * pt1 + t_, pt2) > 0.8) // FoV check
          pts_3d.emplace_back(cv::Point3f(pt1(0), pt1(1), pt1(2)));
      #endif
    }
