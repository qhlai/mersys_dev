...
./server  -server -cert_file:../server.cert -key_file:../server.key
./server -client -unsecure -target:127.0.0.1
    ...
    )
{
    ...
    uint8_t data[SendBufferLength] ={0};
    //
    ...
    memcpy(SendBuffer->Buffer,&data,SendBufferLength);
...
// QUIC_STATUS QUIC_API ReceiveData(
//     _In_ HQUIC Stream,
    ...
    )
{
    ...
    case QUIC_STREAM_EVENT_RECEIVE:{
        ...
        size_t Length = Event->RECEIVE.Buffers->Length;
        // printf("[strm][%p] Data received\n", Stream);
        std::cout <<"Length:"<< Length << std::endl;
        printUint8Array(Data, Length);
        break;
    }    
    ...
    )
{
    ...
    SendBuffer->Buffer = SendBufferRaw + sizeof(QUIC_BUFFER);//跳过 QUIC_BUFFER 结构的大小，指向分配内存块的起始位置
    ...
    printf("[strm][%p] Sending data...\n", Stream);
    std::cout << SendBuffer->Buffer << std::endl;
    // printf("[strm][%p] Sending data...\n", Stream);
    //
    // Sends the buffer over the stream. Note the FIN flag is passed along with
    // the buffer. This indicates this is the last buffer on the stream and the
    ...
    )
{
    ...
    //
    bool unsecure = true;
    if (!ClientLoadConfiguration(unsecure)) {
        return;
    }
    ...
    if ((Target = GetValue(argc, argv, "target")) == NULL) {
        printf("default  '-target' argument 127.0.0.1 !\n");
        Target ="127.0.0.1";
